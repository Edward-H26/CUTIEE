...
def _actionFromBullet(bullet: Bullet) -> Action | None:
    """Whole-plan replay reconstruction. Keeps stored values verbatim.

    Thin wrapper over `bullet_reconstruct.actionFromBullet`; exists so
    existing call sites (`_executeReplay` in the runner) keep their
    single-value return type. Fragment replay calls the canonical
