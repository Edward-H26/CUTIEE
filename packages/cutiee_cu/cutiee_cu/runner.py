...
from .client.gemini_cu import GeminiComputerUseClient
...
class ComputerUseRunner:
    ...
    async def _executePrematchedNodes(self, state: AgentState) -> None:
        ...
        wrong thing.
        """
        # State verification is provided by cutiee_ace (sister package). When
        # cutiee_cu is installed standalone WITHOUT cutiee_ace, fall back to
        # no-op verification — pre-matched nodes still execute, just without
        # the URL + perceptual-hash safety check. This keeps cutiee_cu as a
        # zero-cross-package-dependency library.
        try:
            from cutiee_ace.memory.state_verifier import StateVerifier
            verifier = StateVerifier()
        except ImportError:
            verifier = None
            logger.debug(
                "cutiee_ace not installed; skipping state verification on replay. "
                "Install cutiee_ace for safer mid-task replay."
            )
        ...
        for offset, node in enumerate(self.prematchedNodes):
            ...
            # Phase 4 state verification (only runs if cutiee_ace is installed).
            # Skipped for the very first node since it always runs against the
            # initial page state; verifier handles missing expected_* fields gracefully.
            ...
            action = Action(
                ...
                coordinate = coord,
                reasoning = f"replay-graph: {node.description} ({verification_reason})" if node.description else f"graph-replay ({verification_reason})",
