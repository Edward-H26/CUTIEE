...
```
# buildCommand
uv sync --group browser_use && uv run playwright install chromium && uv run python manage.py migrate --no-input && uv run python manage.py collectstatic --no-input && uv run python -m agent.persistence.bootstrap && cp agent/memory/local_llm_stub.py agent/memory/local_llm.py && rm -f scripts/cache_local_qwen.py tests/agent/test_local_llm.py
...
DJANGO_DATABASE_URL              # PostgreSQL or Render Postgres URL
