...
from agent.memory.subgraph_match import SubgraphMatch, SubgraphMatcher


def _node(actionType: str, target: str = "", value: str = "", coord: tuple[int, int] | None = None) -> ActionNode:
    return ActionNode(
...
def test_subgraph_matcher_finds_full_prefix() -> None:
    ...
    storedNodes = [_node("navigate", target = "https://sheets.com"),
                   _node("click_at", coord = (100, 200)),
                   _node("type_at", value = "=SUM(A:B)")]
    stored = ProcedureGraph(
        procedure_id = "p1", user_id = "alice", task_description = "sheet sum",
        nodes = storedNodes,
    )
    newNodes = [_node("navigate", target = "https://sheets.com"),
                _node("click_at", coord = (100, 200)),
                _node("type_at", value = "=SUM(A:B)")]
    newTask = ProcedureGraph(
        procedure_id = "p2", user_id = "alice", task_description = "sheet sum again",
        nodes = newNodes,
    )
...
def test_subgraph_matcher_finds_partial_prefix() -> None:
    ...
    stored = ProcedureGraph(
        procedure_id = "p1", user_id = "alice", task_description = "old",
        nodes = [_node("navigate", target = "https://x.com"),
                 _node("click_at", coord = (100, 200)),
                 _node("type_at", value = "old value")],
    )
    newTask = ProcedureGraph(
        procedure_id = "p2", user_id = "alice", task_description = "new",
        nodes = [_node("navigate", target = "https://x.com"),
                 _node("click_at", coord = (100, 200)),
...
def test_subgraph_matcher_picks_longest_match_across_multiple_stored() -> None:
    stored1 = ProcedureGraph(
        procedure_id = "p1", user_id = "alice", task_description = "short",
        nodes = [_node("navigate", target = "https://x.com")],
    )
    stored2 = ProcedureGraph(
        procedure_id = "p2", user_id = "alice", task_description = "long",
        nodes = [_node("navigate", target = "https://x.com"),
                 _node("click_at", coord = (10, 10)),
                 _node("type_at", value = "hello")],
    )
    newTask = ProcedureGraph(
        procedure_id = "p3", user_id = "alice", task_description = "task",
        nodes = [_node("navigate", target = "https://x.com"),
                 _node("click_at", coord = (10, 10)),
                 _node("type_at", value = "hello"),
                 _node("finish")],
...
def test_compute_action_hash_handles_none_coords() -> None:
    ...
    h = computeActionHash(
        actionType = "navigate", target = "https://x.com",
        value = "", coordX = None, coordY = None,
    )
